...
function FclAttendanceChart({
    ...
    const fetchChartData = async () => {
      ...
      try {
        ...
        const response = await axiosInstance.get(
          `/fcl/weekly-stats?${params.toString()}`
        );
  ...
  return (
    <Card>
      <Title level={4}>
        Weekly Attendance Chart ({filterDate.format("MMMM YYYY")})
      </Title>
...
export default function FclAdminHome() {
  ...
  const [selectedLeader, setSelectedLeader] = useState<LeaderSummary | null>(
    null
  );
  ...
  const [selectedLeaderName, setSelectedLeaderName] = useState<string | null>(
    null
  );
  ...
  const fetchSummaryData = async (date: dayjs.Dayjs) => {
    ...
    try {
      const response = await axiosInstance.get(
        `/fcl/fcl-summary?month=${date.month() + 1}&year=${date.year()}`
      );
  ...
  useEffect(() => {
    ...
    if (selectedGender) {
      tempFilteredData = tempFilteredData.filter(
        (leader) => leader.gender === selectedGender
      );
    }
    const availableGrades = [
      ...new Set(
        tempFilteredData
          .flatMap((leader) => leader.grade)
          .filter((g) => g != null)
      ),
    ].sort((a, b) => a - b);
    setGradeOptions(
    ...
    if (selectedGrade) {
      tempFilteredData = tempFilteredData.filter(
        (leader) => leader.grade === selectedGrade
      );
    }
    const availableLeaders = [
      ...new Set(tempFilteredData.map((leader) => leader.leaderName)),
    ];
    setLeaderOptions(
      availableLeaders.map((leader) => ({ value: leader, label: leader }))
    );
