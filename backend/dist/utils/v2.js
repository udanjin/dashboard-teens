"use strict";
async;
getAttendance(req, AuthenticatedRequest, res, Response);
{
...
postAttendance(req, AuthenticatedRequest, res, Response);
{
    const leaderId = req.user.userId;
    ...
    try {
        await Promise.all(attendances.map((att) => Attendance.upsert({
            leaderId,
